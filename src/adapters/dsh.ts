...
// here lands inside the budget. Two seconds per spawn keeps a hung bin well
// under it (session-end is ~ms); N served stores pay N spawns.
...
// timeout kills a hung child (status -1, never retried — a retry cannot fix
// a hang). Everything fail-open: a throw, a missing bin, a nonzero exit —
